...
void blink_led()
{
  ...
  {
    digitalWrite(LED_WIFI, !digitalRead(LED_WIFI)); // dao trang thai led se nhap nhay sau khi ket noi
...
void init_wifi()
{
  digitalWrite(LED_WIFI, HIGH);           // cho led sáng lên
...
void reconnect_WiFi()
{
  if (WiFi.status() != WL_CONNECTED) // neu mat ket noi WIFI
  {
    ...
    {
      digitalWrite(LED_WIFI, HIGH);                // neu mat ket noi led sang lun
